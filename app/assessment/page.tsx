...
export default function AssessmentPage() {
  ...
  // 根据单选框状态决定进度：如果选择"完成探索"则进度>=100，否则根据解锁维度计算
  const dimensionProgress = (completedDimensionsCount / DIMENSION_ORDER.length) * 100; // 根据解锁维度计算进度
  const assessmentProgress = reportStatus === "完成探索" ? 100 : dimensionProgress;
  // 根据本地问卷数据计算已完成的维度数
  const completedCount = completedDimensionsCount;
  const totalCount = DIMENSION_ORDER.length; // 总维度数（7个维度）
  ...
  return (
    <PageContainer>
      ...
      <div className="px-4 space-y-4 pb-8 relative z-10 bg-[#F5F5F5]">
        {/* 测试用：是否完成报告的单选框 */}
        ...
        {!isCompleted ? (
          <Card className="px-1.5 py-1.5 gap-1 shadow-2xl bg-gradient-to-r from-[#1A4099]/5 to-white border-2 border-[#1A4099]/30">
            ...
            <div className="relative h-1.5 my-1.5 bg-gray-200 rounded-full overflow-hidden">
              <div
          ...
          <Card className="p-3 shadow-2xl bg-gradient-to-br from-[#1A4099]/10 to-[#FF7F50]/10 border-2 border-[#1A4099]/30">
            <div className="flex items-center gap-1.5 mb-1.5">
              <div className="w-9 h-9 rounded-full bg-[#FF7F50]/20 flex items-center justify-center flex-shrink-0">
            ...
            <Link href="/assessment/all-majors">
              <Button
                className="w-full h-8 bg-[#FF7F50] hover:bg-[#E66A42] text-white shadow-md mb-1.5"
                size="sm"
              >
                查看问卷 <ArrowRight className="w-3 h-3 ml-1" />
              </Button>
            </Link>
          </Card>
        )}
